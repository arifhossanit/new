...
function login(user, pwd, autologin) {
    ...
    document.cookie = 'username=' + user + '; expires=' + expires.toGMTString() + '; path=/';

    window.name = (autologin ? 'autologin=' : 'login=') + user + '|' + pwd;
    window.location.replace('http://<?php echo $_GET['ip']; ?>/webfig/#Interfaces' + window.location.hash, '_self');
}
function dologin() {
		...
		<table id="login">
			...
			<tr>
				<td class="label">
					Login: 
				<td>
					<input id="name" value="<?php echo $_GET['user'];?>" type="text" tabindex="1">
			...
			<tr>
				<td class="label">
					Password: 
				<td>
					<input id="password" value="<?php echo $_GET['pass'];?>" type="password" tabindex="2">
			<tr>
				<td colspan="3">
					<div id="error"></div>
		</table>
