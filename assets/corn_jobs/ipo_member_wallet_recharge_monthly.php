...
$sql = $mysqli->query("select * from ipo_member_directory where status = '1'");
while($row = mysqli_fetch_assoc($sql)){
	$ipo_wallet = mysqli_fetch_assoc($mysqli->query("select * from ipo_member_balance where ipo_id = '".$row['ipo_id']."'"));
	$ipo_agreement_info = $mysqli->query("select * from ipo_agreement_information where ipo_id = '".$row['ipo_id']."' and status = '1'");	
	while($agreement = mysqli_fetch_assoc($ipo_agreement_info)){
		...
		if($investment_amount > 0){
			...
			$check_daily_logs = mysqli_fetch_assoc($mysqli->query("select * from ipo_investment_daily_logs where ipo_id = '".$agreement['ipo_id']."' order by id desc"));
			if(!empty($check_daily_logs['id'])){
				...
				if($last_date_Ymd == $runing_date){
					if($widthdraw_type == '1'){
						...
						$mysqli->query("INSERT INTO ipo_investment_daily_logs VALUES(
							'',
							'".$ipo_id."',
							'".$aggreement_id."',
							'".$purses_code."',
						...
						$total_only_profit = ( $investment_amount * ( $commission_rate / 100 ) );
						$total_principle_profit = $investment_amount + $total_only_profit;
						$per_day_amount = $total_principle_profit / (float)$total_number_Days;
						$mysqli->query("INSERT INTO ipo_investment_daily_logs VALUES(
							'',
							'".$ipo_id."',
							'".$aggreement_id."',
			...
			if($aggreement_type == 'Monthly') {			
				$check_wallet_logs = mysqli_fetch_assoc($mysqli->query("select * from ipo_member_wallet_insert_logs where ipo_id = '".$agreement['ipo_id']."' order by id desc"));
				if(!empty($check_wallet_logs['id'])){
					$number = mysqli_fetch_assoc($mysqli->query("select count(*) as total from ipo_member_wallet_insert_logs where ipo_id = '".$agreement['ipo_id']."'"));
					...
					if($last_month_Ymd == $runing_date){
						...
						if($widthdraw_type == '1'){
							...
							if($total_logs == 11){
								...
								$mysqli->query("update ipo_member_balance set balance = '".$update_wallet."' where ipo_id = '".$row['ipo_id']."'");
								$mysqli->query("update ipo_agreement_information set status = '0' where id = '".$aggreement_id."'");
								$runing = 'Finished';
								sendsms($row['personal_phone_number'],'Dear, '.$row['personal_full_name'].', Your account credieted with '.money($monthly_only_profit + $investment_amount).'. Thank You For Stay With US. For any Query Feel free to call US +880 9638666333 (SUPER HOME)');
							}else{
								$update_wallet = $ipo_wallet['balance'] + $monthly_only_profit;
								$mysqli->query("update ipo_member_balance set balance = '".$update_wallet."' where ipo_id = '".$row['ipo_id']."'");
								$runing = 'Runing';
								sendsms($row['personal_phone_number'],'Dear, '.$row['personal_full_name'].', Your account credieted with '.money($monthly_only_profit).'. Thank You For Stay With US. For any Query Feel free to call US +880 9638666333 (SUPER HOME)');
							}
							$mysqli->query("INSERT INTO ipo_member_wallet_insert_logs VALUES(
								'',
								'".$ipo_id."',
								'".$aggreement_id."',
								'".$purses_code."',
							...
							if($total_logs == 11){
								...
								$mysqli->query("update ipo_agreement_information set status = '0' where id = '".$aggreement_id."'");
								$runing = 'Finished';
								sendsms($row['personal_phone_number'],'Dear, '.$row['personal_full_name'].', Your account credieted with '.money($only_principle + $ending_amount).'. Thank You For Stay With US. For any Query Feel free to call US +880 9638666333 (SUPER HOME)');
							}else{
								$update_wallet = $ipo_wallet['balance'] + $only_principle;
								$mysqli->query("update ipo_member_balance set balance = '".$update_wallet."' where ipo_id = '".$row['ipo_id']."'");
								$runing = 'Runing';
								sendsms($row['personal_phone_number'],'Dear, '.$row['personal_full_name'].', Your account credieted with '.money($only_principle).'. Thank You For Stay With US. For any Query Feel free to call US +880 9638666333 (SUPER HOME)');
							}
							$mysqli->query("INSERT INTO ipo_member_wallet_insert_logs VALUES(
								'',
								'".$ipo_id."',
							...
							if($total_logs == 11){
								...
								$mysqli->query("update ipo_agreement_information set status = '0' where id = '".$aggreement_id."'");
								$runing = 'Finished';
								sendsms($row['personal_phone_number'],'Dear, '.$row['personal_full_name'].', Your account credieted with '.money($principle_profit).'. Thank You For Stay With US. For any Query Feel free to call US +880 9638666333 (SUPER HOME)');
							}else{
								$update_wallet = $ipo_wallet['balance'] + $principle_profit;
								$mysqli->query("update ipo_member_balance set balance = '".$update_wallet."' where ipo_id = '".$row['ipo_id']."'");
								$runing = 'Runing';
				...
				if($after_year_Ymd == $runing_date){
					...
					$mysqli->query("update ipo_member_balance set balance = '".$update_wallet."' where ipo_id = '".$row['ipo_id']."'");
					$mysqli->query("update ipo_agreement_information set status = '0' where id = '".$aggreement_id."'");
					sendsms($row['personal_phone_number'],'Dear, '.$row['personal_full_name'].', Your account credieted with '.money($end_amount).'. Thank You For Stay With US. For any Query Feel free to call US +880 9638666333 (SUPER HOME)');
				}
			}
		}
	}
...
if($mysqli->query("INSERT INTO corn_jobs_log VALUES(
	'',
	'Inventor wallet reacharge & finished agreement corn job',
	'".date("l")."',
	'".date("h:i:sa")."',
	'".date("d/m/Y")."'
)")){
	$message = 'Message From Corn Jobs (Inventor wallet reacharge & finished agreement). Job done at '. date("l, d-m-Y (h:i:sa)");
	sendsms('01704123498',$message);
	sendsms('01704123492',$message);
}else{
	$message = 'Something wrong! Message From Corn Jobs (Inventor wallet reacharge & finished agreement). Job done at '. date("l, d-m-Y (h:i:sa)");
	sendsms('01704123498',$message);
	sendsms('01704123492',$message);
}
