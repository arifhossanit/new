...
$meq = $mysqli->query("SELECT * FROM member_directory WHERE status = '3'");
$mem_booking_info = '';
while($mow = mysqli_fetch_assoc($meq)){
	$mem_booking_info .= "'".$mow['booking_id']."',";
}
$check_out_mem = rtrim($mem_booking_info,',');
$where = "rent_status = 'Due' ".$branch_user." $month_filter AND booking_id NOT IN (".$check_out_mem.")";
$columns = array(
    array( 'db' => 'id', 'dt' => 0 ),
