...
$where = "status in (1,2,3)".$branch_id.$date_filter." ORDER BY id DESC";
$columns = array(
    array( 'db' => 'id',   'dt' => 0 ),
    array(
		'db' => 'branch_id',
    ...
    array(
		...
		'formatter' => function( $d, $row ) {global $mysqli;
            // return $d;
            $returned_money = mysqli_fetch_assoc($mysqli->query("SELECT id, amount from return_diposit_money where booking_id LIKE '".$d."'"));
            if(!is_null($returned_money)){
                return $returned_money['amount'];
            }else{
                return 0;
            }
		}
    ...
    array(
		...
		'formatter' => function( $d, $row ) {global $mysqli;
            // return $d;
            $returned_money = mysqli_fetch_assoc($mysqli->query("SELECT id, data from return_diposit_money where booking_id LIKE '".$d."'"));
            if(!is_null($returned_money)){
                return $returned_money['data'];
            }else{
                return 'Due';
            }
		}
    ...
    array(
		...
		'formatter' => function( $d, $row ) {global $mysqli;
            // return $d;
            $cancel_report = mysqli_fetch_assoc($mysqli->query("SELECT id from cencel_request where booking_id LIKE '".$d."' and note = 'Request For Cancel for rental payment issue (auto cancel from software)'"));
            if(!is_null($cancel_report)){
                return 'Yes';
            }else{
                return 'No';
            }
    ...
    array(
		...
		'formatter' => function( $d, $row ) {global $mysqli;
            ...
            $cancel_report = mysqli_fetch_assoc($mysqli->query("SELECT id from cencel_request where booking_id LIKE '".$d."' and note = 'Request For Cancel for rental payment issue (auto cancel from software)'"));
            if(!is_null($cancel_report)){
                $package_price = mysqli_fetch_assoc($mysqli->query("SELECT package_price from packages where id = ".$row[10]));
                return $package_price['package_price'];
            }else{
                return 0;
            }
		}
