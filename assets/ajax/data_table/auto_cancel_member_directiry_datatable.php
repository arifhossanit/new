...
if(!empty($condition_check['id'])){
	...
	while($mow = mysqli_fetch_assoc($mql)){
		$cenc = mysqli_fetch_assoc($mysqli->query("select * from cencel_request where booking_id = '".$mow['booking_id']."' AND note IN ('Request For Cancel for rental payment issue (auto cancel from software)')"));
...
$where = "booking_id IN (".$fbooking_id.")"; // 	return type == 'export' ? meta.row + 1 : data;
*/
...
$where = "$branch_user booking_id IN (SELECT booking_id FROM cencel_request WHERE cencel_request.booking_id = member_directory.booking_id AND note IN ('Request For Cancel for rental payment issue (auto cancel from software)')) and status = '1' $m_type";
$columns = array(
    array( 'db' => 'id', 'dt' => 0 ),
	array( 
		'db' => 'photo_avater', 
