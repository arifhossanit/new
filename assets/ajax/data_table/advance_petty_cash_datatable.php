...
$columns = array(
	...
	array(
        ...
        'formatter' => function( $d, $row ) {			
			...
			return '<span style="white-space: nowrap;">'.$employee['department_name'].'</span>';
        }
    ),
	array(
        'db'        => 'amount',
		...
		'formatter' => function( $d, $row ){
			if($d == 1){
				return '<span style="white-space: nowrap;">
					...
					<button type="button" onclick="return resend_balance_accepted_otp('.$row[2].')" class="btn btn-xs btn-success">Resend OTP!</button>
				</span>';
			}else{
				return '
