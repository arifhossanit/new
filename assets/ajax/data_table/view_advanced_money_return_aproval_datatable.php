...
$columns = array(
	...
	array(
        ...
        'dt'        => 3,
        'formatter' => function( $d, $row ) {		
			return '<span style="font-weight:bolder;color:red;">'.money($d).'</span>';
        }
    ),
	array(
        'db'        => 'id',
        ...
        'formatter' => function( $d, $row ) { global $mysqli;
			...
			if($logs['aproval'] == 1){
				return '<span style="font-weight:bolder;color:green;">'.money($logs['rest_of_amount']).'</span>';
			}else{
				return 'Pending--';
			}
	...
	array(
		...
		'formatter' => function( $d, $row ) { global $mysqli;			
			if(!empty($d)){
				$logs = mysqli_fetch_assoc($mysqli->query("select * from advance_petty_cash_return_logs where id = '".$d."'"));
				if($logs['aproval'] == 1){
					$aproval_data = mysqli_fetch_assoc($mysqli->query("select * from advance_return_aproval_logs where return_id = '".$logs['id']."'"));
					
					$u = explode("___",$aproval_data['uploader_info']);
					if(!empty($u[0]) AND !empty($u[1])){
						$emp = mysqli_fetch_assoc($mysqli->query("SELECT * FROM employee WHERE email = '".$u[1]."'"));
						return ''.$emp['full_name'].'';
					}
    ...
    array(
        ...
        'formatter' => function( $d, $row ) {
			...
			if($logs['aproval'] == 2){
				...
				}else if ($logs['aproval'] == 0){
					$data .= '<button onclick="return approval_button(1,'.$logs['id'].')" type="button" class="btn btn-xs btn-success" style="margin-left:5px;">Approve</button>';
					$data .= '<button onclick="return approval_button(2,'.$logs['id'].')" type="button" class="btn btn-xs btn-danger" style="margin-left:5px;">Reject</button>';
				}
			}			
			return $data;
        }
