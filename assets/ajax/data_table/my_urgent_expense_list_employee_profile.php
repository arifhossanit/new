    ...
    array(
        ...
        'formatter' => function( $d, $row ) {
			...
			if($logs['approval'] == 3){
				...
				if ($logs['approval'] == 0){
					$data .= '<button type="button" class="btn btn-xs btn-warning" style="margin-left:5px;">Pending...!</button>';
					$data .= '<button  onclick="return purses_item_checkit_widthdraw('.$transaction_id .','.$money.','.$name.')" type="button" class="btn btn-xs btn-danger" style="margin-left:5px;">Widthdraw!</button>';
			...
			$data .= ' <button onclick="return show_receipt('.$transaction_id .')" type="button" class="btn btn-xs btn-info"><i class="fa fa-eye"></i></button>';
			return $data;
        }
    )	
);
