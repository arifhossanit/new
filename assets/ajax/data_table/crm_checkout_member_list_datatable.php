...
$pci = '';
$sqll = $mysqli->query("select id from packages where try_us = '1'");
while($pc = mysqli_fetch_assoc($sqll)){
	if(!empty($pc['id'])){
		$pci .= "'".$pc['id']."',";
	}	
}
$pci = rtrim($pci,",");
$where = "status = '3' ".$branch_user." AND note = '' and package in (".$pci.")"; 
$columns = array(
    array( 'db' => 'id', 'dt' => 0 ),
	array( 
		'db' => 'photo_avater', 
    ...
    array(
        ...
        'formatter' => function( $d, $row ) {
			...
			if(!empty($confirm_ccheck['id']) AND $confirm_ccheck['status'] == '1'){
				$data .= '<button onclick="return view_member_profile('.$d.')" type="button" class="btn btn-xs btn-success" title=""><i class="fas fa-money-check-alt"></i>&nbsp;&nbsp;&nbsp;Return Deposit Money</button>';
			}else{
				if(!empty($chk['data'])){
					...
					if($Datee <= '20210331'){
						...
						$data .= '<button type="button" class="btn btn-xs btn-danger" title=""><i class="fas fa-money-check-alt"></i>&nbsp;&nbsp;&nbsp;Need Member Approval</button>';
					}
				}else{
					$data .= '<button onclick="return view_member_profile('.$d.')" type="button" class="btn btn-xs btn-success" title=""><i class="fas fa-money-check-alt"></i>&nbsp;&nbsp;&nbsp;Return Deposit Money</button>';
				}				
