...
$products = $mysqli->query("SELECT scm_product_types.name as type_name, scm_products.product_name, scm_product_category.name as category_name, scm_scales.name as scale_name, scm_brands.name as brand_name, scm_product_requisition_details.* FROM scm_product_requisition_details
...
ORDER by scm_product_requisition_details.id desc");
// var_dump($products);
...
while($product = mysqli_fetch_assoc($products)){
    ...
    if($product['type_name'] == 'Storable'){
        ...
        $html .= '<td>
                    <div class="row">
                        <div class="col-md-6">';
        if(isset($_SESSION['stolen_product_department_receive_from_department'][$product['id']])){
            $html .= '<input type="hidden" class="form-control" name="stolen_amount[]" value="'.count($_SESSION['stolen_product_department_receive_from_department'][$product['id']]).'"><p>'.count($_SESSION['stolen_product_department_receive_from_department'][$product['id']]).'</p>';
        }else{            
        ...
        $html .= '<td>
                    <div class="row">
                        <div class="col-md-6">';
        if(isset($_SESSION['damaged_product_department_receive_from_department'][$product['id']])){
            $html .= '<input type="hidden" class="form-control" name="damaged_amount[]" value="'.count($_SESSION['damaged_product_department_receive_from_department'][$product['id']]).'"><p>'.count($_SESSION['damaged_product_department_receive_from_department'][$product['id']]).'</p>';
        }else{            
        ...
        if((int)$number_of_row['num_row'] > 1){
            ...
            $html .= '<td>
                    <div class="row">
                        <div class="col-md-6">';
            if(isset($_SESSION['stolen_product_department_receive_from_department'][$product['id']])){
                $html .= '<input type="hidden" class="form-control" name="stolen_amount[]" value="'.array_sum($_SESSION['stolen_product_department_receive_from_department'][$product['id']]).'"><p>'.array_sum($_SESSION['stolen_product_department_receive_from_department'][$product['id']]).'</p>';
            }else{            
                $html .= '<input type="hidden" class="form-control" name="stolen_amount[]" value="0"><p>0</p>';
            }
            $html .=        '</div>
            ...
            $html .= '<td>
                    <div class="row">
                        <div class="col-md-6">';
            if(isset($_SESSION['damaged_product_department_receive_from_department'][$product['id']])){
                $html .= '<input type="hidden" class="form-control" name="damaged_amount[]" value="'.array_sum($_SESSION['damaged_product_department_receive_from_department'][$product['id']]).'"><p>'.array_sum($_SESSION['damaged_product_department_receive_from_department'][$product['id']]).'</p>';
            }else{            
                $html .= '<input type="hidden" class="form-control" name="damaged_amount[]" value="0"><p>0</p>';
            }
            $html .=        '</div>
