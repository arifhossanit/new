...
while($product = mysqli_fetch_assoc($products)){
    ...
    if($stocks){
        while($stock = mysqli_fetch_assoc($stocks)){
            $temp = mysqli_fetch_assoc($mysqli->query("SELECT count(DISTINCT(warehouse_id)) as id_count, SUM(stock_amount) as received_amount from scm_warehouse_product_stock where scm_product_order_details_id = ".$stock['id']." GROUP BY scm_product_order_details_id"));
