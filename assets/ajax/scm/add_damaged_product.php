...
if(!isset($_SESSION['super_admin']['email'])){
    ...
    if($_POST['product_type'] == 'consumable'){
        foreach($_POST['receive_id'] as $idx=>$receive_id){
            $_SESSION['damaged_product_department_receive'][$_POST['product_id']][$receive_id] = ($_POST['damaged_product'][$idx] == '') ? 0 : (int)$_POST['damaged_product'][$idx];
        }
    }else{
        if(isset($_POST['damaged_product'])){
            $_SESSION['damaged_product_department_receive'][$_POST['product_id']] = $_POST['damaged_product'];
        }else{
