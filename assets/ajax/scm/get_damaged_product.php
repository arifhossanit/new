...
if(!isset($_SESSION['super_admin']['email'])){
    ...
    if($_POST['type'] == 'consumeable'){
        ...
        $html = '   <input type="hidden" id="product_id_damaged" value="'.$_POST['id'].'">
        ...
        while($product = mysqli_fetch_assoc($products)){
            /*
                Getting max amount for, when any stolen amount is set.
            */
            $max_amount = $product['amount'];
            if(isset($_SESSION['stolen_product_department_receive'][$_POST['id']])){
                if(isset($_SESSION['stolen_product_department_receive'][$_POST['id']][$product['id']])){
                    $max_amount = $product['amount'] - $_SESSION['stolen_product_department_receive'][$_POST['id']][$product['id']];
                }
            }
            $amount = '';
            if(isset($_SESSION['damaged_product_department_receive'][$_POST['id']][$product['id']])){
                $amount = $_SESSION['damaged_product_department_receive'][$_POST['id']][$product['id']];
            }
            $html .= '<tr>';
            $html .= '<td>'.$product['purchase_order_id'].'</td>';
            $html .= '<td>'.$product['product_name'].'</td>';
            $html .= '<td>'.$product['warehouse_name'].'</td>';
            $html .= '<td><input type="hidden" name="receive_id[]" value="'.$product['id'].'"><input type="number" step="any" class="form-control" name="damaged_product[]" max="'.$max_amount.'" value="'.$amount.'" placeholder="Maximum Amount: '.$max_amount.'"></td>';
            $html .= '</tr>';
        }
        $html .= '<tbody>
                </table>';
        ...
        $html = '<div class="row">
                    <input type="hidden" id="damage_type" value="storable">
                    <input type="hidden" id="product_id_damaged" value="'.$_POST['id'].'">';
        $idx = 0;
        while($barcode = mysqli_fetch_assoc($barcodes)){
            $idx++;
            /*
                Making the options readonly that are stolen.
            */
            if(isset($_SESSION['stolen_product_department_receive'][$_POST['id']])){
                if(array_search($barcode['id'],$_SESSION['stolen_product_department_receive'][$_POST['id']]) !== false){
            ...
            if(isset($_SESSION['damaged_product_department_receive'][$_POST['id']])){
                if(array_search($barcode['id'],$_SESSION['damaged_product_department_receive'][$_POST['id']]) !== false){
                    $html .= '  <div class="col-md-4">
                                    <div class="form-check">
                                        <input name="damaged_product[]" class="form-check-input regular-checkbox" type="checkbox" value="'.$barcode['id'].'" id="flexCheckDefault_'.$idx.'" checked>
                                        <label style="font-size: 25px;" class="form-check-label ml-2" for="flexCheckDefault_'.$idx.'">
                                            '.$barcode['barcode'].'
                                        </label>
                                    </div>
            ...
            $html .= '  <div class="col-md-4">
                                    <div class="form-check">
                                        <input name="damaged_product[]" class="form-check-input regular-checkbox" type="checkbox" value="'.$barcode['id'].'" id="flexCheckDefault_'.$idx.'">
                                        <label style="font-size: 25px;" class="form-check-label ml-2" for="flexCheckDefault_'.$idx.'">
                                            '.$barcode['barcode'].'
                                        </label>
                                    </div>
