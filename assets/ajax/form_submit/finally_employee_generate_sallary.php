	...
	if(!empty($check_year_month['total_salaray_generate']) AND $check_year_month['total_salaray_generate'] == 2 ){
		...
		if(isset($_POST['final_generation'])){
			...
			while($row = mysqli_fetch_assoc($get_employee)){
				$check_leave = $mysqli->query("select * from employee_leave_logs WHERE employee_id = '".$row['employee_id']."' and ( aproval = '1' OR (h_id in ('113', '114')))");
				$get_leave_date = "";
				...
				if(!empty($check_increament['id'])){
					...
					while($att_row = mysqli_fetch_assoc($att_ddat_fest)){
						...
						while($inc_row = mysqli_fetch_assoc($increament_logs)){
							...
							$attendence_date = new DateTime(date('Y-m-d H:i:s', strtotime($year.'-'.$month.'-'.sprintf("%02d",$att_row['days'].' 00:00:00'))));
							$increment_start_date = date_create_from_format('d/m/Y H:i:s', $inc_row['start_date'].' 00:00:00');
							$get_half_day = $mysqli->query("SELECT employee_everyday_leave_logs.id from employee_everyday_leave_logs INNER JOIN employee_leave_logs using(unique_id) where employee_everyday_leave_logs.days = '".$attendence_date->format('d')."' AND employee_everyday_leave_logs.month = '".$attendence_date->format('m')."' AND employee_everyday_leave_logs.year = '".$attendence_date->format('Y')."'  AND employee_leave_logs.employee_id = '".$row['employee_id']."' AND employee_everyday_leave_logs.status = 1 AND how_many_days = '0.5'");
							
							if($increment_start_date <= $attendence_date){
								if($get_half_day->num_rows > 0){
									$increament_amount = $increament_amount + ($inc_row['amount'] / 2);
								}else{
				...
				if(!empty($check_decreament['id'])){
					...
					while($att_row = mysqli_fetch_assoc($att_ddat_fest)){
						...
						while($dec_row = mysqli_fetch_assoc($decreament_logs)){
							$attendence_date = DateTime::createFromFormat('Y-m-d H:i:s', $year.'-'.$month.'-'.sprintf("%02d",$att_row['days']).' 00:00:00');
							$decrement_start_date = DateTime::createFromFormat('d/m/Y H:i:s', $dec_row['start_date'].' 00:00:00');
							
							$get_half_day = $mysqli->query("SELECT employee_everyday_leave_logs.id from employee_everyday_leave_logs INNER JOIN employee_leave_logs using(unique_id) where employee_everyday_leave_logs.days = '".$attendence_date->format('d')."' AND employee_everyday_leave_logs.month = '".$attendence_date->format('m')."' AND employee_everyday_leave_logs.year = '".$attendence_date->format('Y')."'  AND employee_leave_logs.employee_id = '".$row['employee_id']."' AND employee_everyday_leave_logs.status = 1 AND how_many_days = '0.5'");
							
							if($decrement_start_date <= $attendence_date){
								if($get_half_day->num_rows > 0){
									$decreament_amount = $decreament_amount + ($dec_row['amount'] / 2);
								}else{
				...
				$last_working_date_query = "";
				if(!is_null($row['last_working_date'])){
					$last_working_date_query = " AND STR_TO_DATE(CONCAT(days,'-',IF(LENGTH(month) = 1, CONCAT('0', month), month),'-',years), \"%e-%m-%y\") <= '".substr($row['last_working_date'], 0, 10)."' ";
				}
				$get_employee_attendance_data = mysqli_fetch_assoc($mysqli->query("SELECT count(DISTINCT days) as total_present FROM employee_attendence WHERE employee_id = '".$row['employee_id']."' AND attendance = '1' AND month = '".$month_attendance."' AND years = '".$year_attendance."' $last_working_date_query ".$leave_query.""));	
			...
			$info = array(
				'html' => $html,
				'total' => $total_salary,
				'button' => '<button style="float:right;" type="button" class="btn btn-primary" onclick="send_otp()">Password</button>'
			);
			echo json_encode($info);
		}					
	}
