...
if(isset($_POST['form_submit'])){ 
	$cash_transaction = '0';
	$member_info = mysqli_fetch_assoc($mysqli->query("select * from ipo_member_widthdraw_request where id = '".$_POST['member_id']."'"));
	$member_data = mysqli_fetch_assoc($mysqli->query("select * from ipo_member_directory where ipo_id = '".$member_info['ipo_id']."'"));
	$ipo_balance = mysqli_fetch_assoc($mysqli->query("select * from ipo_member_balance where ipo_id = '".$member_info['ipo_id']."'"));
	...
	$p_booking_id = $member_data['ipo_id'];
	$p_uploader_info = $_POST['uploader_info'];
	$p_table = 'ipo_aproved_widthdraw_money_logs';
	...
	$transaction = "insert into transaction values(
		...
		'".$mysqli->real_escape_string($member_data['ipo_id'])."',
		'".$mysqli->real_escape_string($member_data['personal_full_name'])."',
		'Defult',
		'Defult',
	...
	$diposit_submit = "insert into ipo_aproved_widthdraw_money_logs values(
		'',
		'".$mysqli->real_escape_string($member_data['ipo_id'])."',
		'".$mysqli->real_escape_string($_POST['money'])."',
		'".$mysqli->real_escape_string($payment_method)."',
	...
	if($cash_transaction == 1){
		if($_POST['money'] <= 1000){
			$get_petty = mysqli_fetch_assoc($mysqli->query("SELECT * FROM branch_petty_cash WHERE branch_id = '".$bc['branch_id']."'"));
			if(!empty($get_petty['branch_id'])){
				...
				$mysqli->query("INSERT INTO instant_transaction_logs VALUES(
					'',
					'".$transaction_idO."',
					'".$bc['branch_id']."',
					'".(int)$_POST['money']."',
	...
	$ipo_total = (float)$ipo_balance['balance'] - (float)$_POST['money'];
	$ipo_balance_update = "update ipo_member_balance set balance = '".$ipo_total."' where ipo_id = '".$member_data['ipo_id']."'";
	if(
		...
		AND
		$mysqli->query($ipo_balance_update)
		AND
		$mysqli->query("UPDATE ipo_member_widthdraw_request SET status = '2' WHERE id = '".$member_info['id']."'")
	){
		if($_POST['money'] <= 1000){
		...
		if(sendsms($member_data['personal_phone_number'],$message)){
			if($_POST['payment_method'][0] == 'Check'){
				...
				$mysqli->query("insert into check_print_data values(
					...
					'".$mysqli->real_escape_string($member_data['card_number'])."',
