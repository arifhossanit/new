...
<div class="row">
	<div class="col-sm-12">
		<h1 style="text-align:center;color:#f00;">Account Unauthorized! Please Athorized It Before Shifting</h1>
	</div>	
</div>		
<?php }else{ 
...
<div class="row">
	<div class="col-sm-12">
		...
		<!-- <div class="col-md-12">
			...
			<div class="row">
				<div class="col-md-4">
					<p class="p-0 m-0">Recharge Days: <span id="recharge_days"></span></p>
					<p class="p-0 m-0">Days Stayed: <span id="days_stayed"></span></p>
					<p class="p-0 m-0">Available Days: <span id="available_days"></span></p>
					<p class="p-0 m-0">Current Package Rent: <span id="currrent_rent"></span></p>
				</div>
				<div class="col-md-4">
					<p class="p-0 m-0">New Package Rent: <span id="new_rent"></span></p>					
				</div>
			</div>
		</div> -->
	</div>
	...
	if(bed_id != ''){
		...
		alert('Something wrong! Please contact with IT Department.');
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="form-group">
								<button type="submit" name="finally_package_change" id="finally_bed_change" class="btn btn-success" style="float:right;">Change Package</button>
							</div>
						</div>
					</div>
...
$(document).on('keyup', '.package_change_payment_container input[placeholder="Amount"]', function (event, data) {
	var written_amount = 0;
	$('.package_change_payment_container input[placeholder="Amount"]').each(function(){
		if($(this).val() != ''){
			written_amount += parseInt($(this).val());
		}		
	})
	var due_result_amount_booking = written_amount - parseInt($('#total_amount_get').val());
	$("#due_result_amount_package_shift").html('Calculation: ' + due_result_amount_booking);	
	if(parseInt($('#total_amount_get').val()) <= written_amount){
...
function monthly_rent_calculator (selected_package) {
	...
	$.ajax({  
		...
		success:function(data){
			...
			console.log('Given rent: ' + info.given);
			console.log('New rent: ' + info.new);
			console.log('Net payable: ' + info.net_payable);
			...
			$('#total_amountbed_packge_shifting').html(( parseInt($('#total_amount_get').val()) + parseInt(info.net_payable) ));		
			// $('#bed_result').html(data); 
			// $('#bed_info_header').html('<i class="fas fa-check"></i> Selected Bed Information. Bed Type: '+bed_typ_sh);
			// $('#Shifting_model').modal('hide');
			// $('#bed_selecting_model').modal('show');   
