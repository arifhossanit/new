											...
											<div class="row">
												...
												<div class="col-sm-2">
													<div class="form-group">
														...
														<select id="package_type" class="form-control" onchange="return booking_report_table()">
															<option value="all">All</option>
															<option value="try_us">Try Us</option>
...
function booking_report_table(){
	...
	var filter = '?branch_id='+$("#branch_id_hrad").val()+'&date_range='+date_range+'&payment_pattern='+$("#payment_pattern").val()+'&package_type='+$("#package_type").val();
	console.log(filter);
	var ajax_data = '<?=base_url(); ?>assets/ajax/data_table/report/electricity_bill_report_datatable.php'+filter;
...
$('document').ready(function(){
	...
	var table_booking = $('#booking_data_table').DataTable({
		...
		"processing": true,
        "serverSide": false,
        "ajax": "<?=base_url(); ?>assets/ajax/data_table/report/electricity_bill_report_datatable.php"+filter,
		dom: 'lBfrtip',
        buttons: [			
            {
                extend: 'copy',
		...
		"footerCallback": function(row, data, start, end, display) {
			...
			$( api.column( 6 ).footer() ).html(
                formatter.format(total_rent)
            );
			$( api.column( 7 ).footer() ).html(
                formatter.format(total_electricity)
            );
			$( api.column( 8 ).footer() ).html(
                formatter.format(total_parking)
            );
			$( api.column( 9 ).footer() ).html(
                formatter.format(total_penalty)
            );
			$( api.column( 10 ).footer() ).html(
                formatter.format(total_card)
            );
			$( api.column( 11 ).footer() ).html(
                formatter.format(total_tea)
            );
			$( api.column( 12 ).footer() ).html(
                formatter.format(total_locker)
            );
			$( api.column( 13 ).footer() ).html(
                formatter.format(total_receive)
            );
			$( api.column( 14 ).footer() ).html(
                formatter.format(remainder)
            );
			// api.columns('.sum', {
			// 	page: 'all'
			// }).every(function() {
