...
<div class="content-wrapper">
	<div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Checkin Report</h1>
          </div> 
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="<?=base_url();?>">Home</a></li>
              <li class="breadcrumb-item active">Checkin Report</li>
            </ol>
          </div> 
        </div> 
      </div> 
								...
								<div class="card-body">
									<div class="row">
										<div class="col-sm-12">
											<div class="row">
												<div class="col-sm-3">
                                                    <label>Select Checkin Date(mm/dd/yyyy)</label>
                                                    <div class="row">
                                                        <div class="col-sm-2">
                                                            <label>From</label>
                                                        </div>
												...
												<div class="col-sm-2">
													<div class="form-group">
														...
														<select id="package_name" onchange="return booking_report_table()" name="package_name" class="form-control select2">
															<option value="1">All</option>
															<?php
															if(!empty($package_name)){
																foreach($package_name as $row){
																	echo '<option>'.$row->sub_package_name.'</option>';
												...
												<div class="col-sm-2">
													<div class="form-group">
														<label>Select Cancel Type</label>
														<select id="cancel_type" class="form-control" onchange="return booking_report_table()">
															<option value="">All</option>
															<option value="1">Auto Cancel</option>
														</select>
													</div>
												</div>
											</div>
...
function booking_report_table(){
	...
	var filter = '?branch_id='+$("#branch_id_hrad").val()+'&date_range='+date_range+'&payment_pattern='+$("#payment_pattern").val()+'&package_name='+$("#package_name").val()+'&cancel_type='+$("#cancel_type").val();
...
$('document').ready(function(){
	...
	var table_booking = $('#booking_data_table').DataTable({
		...
		"processing": true,
        "serverSide": true,
        "ajax": "<?=base_url(); ?>assets/ajax/data_table/report/checkin_report_datatable.php"+filter,
		dom: 'lBfrtip',
        buttons: [			
            {
                extend: 'copy',
