<div class="content-wrapper">
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0 text-dark">Today Candidate Attendance</h1>
				</div> 
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						...
						<li class="breadcrumb-item active">Today Candidate Attendance</li>
					</ol>
				</div> 
			</div> 
		</div> 
				...
				<div class="col-sm-12">
					<div class="card card-success">
						<div class="card-header">
							<h3 class="card-title">Today candidate list from visitor book</h3>
							<div id="export_buttons_due" style="float: right;"></div>
						</div>
						<div class="card-body">
							<span id="message_from_server"></span>
							<input type="date" id="check_date" value="<?php echo date('Y-m-d');?>" class="form-control" style="width:200px;position: absolute;"/>
							...
							<table id="candidate_list" class="display table table-sm table-bordered table table-striped" style="width:100%;font-size:16px;">
								<thead>
									<tr>
										...
										<th>Time</th>
										<th>Date</th>
										<th style="min-width:280px;max-width:280px;width:280px;s">Option & Star Mark</th>
									</tr>
								</thead>
								<tbody>									
								</tbody>
...
$('document').ready(function(){
	...
	$("#check_date").on("keyup keydown change onclick",function(){
		var condition = '?check_date='+$("#check_date").val();
		var ajax_data = "<?=base_url('assets/ajax/data_table/hrm/recruitment/today_candidate_attendance_datatable_data.php'); ?>"+condition;
		$('#candidate_list').DataTable().ajax.url(ajax_data).load();
	})
	var condition = '?check_date='+$("#check_date").val();
	var table = $('#candidate_list').DataTable({
		...
		"processing": true,
        "serverSide": true,
        "ajax": "<?=base_url('assets/ajax/data_table/hrm/recruitment/today_candidate_attendance_datatable_data.php'); ?>"+condition,
		dom: 'lBfrtip',
        buttons: [			
            {
                extend: 'copy',
