						...
						<div class="card-body">
							<div class="row">
								<div class="col-md-2 mb-3">
									<input id="month" class="form-control" type="month" value="<?php $date = new DateTime('last day of last month'); echo $date->format('Y-m');?>" onchange="change_month(this.value)">
								</div>
								<div class="col-md-12">
	...
	if(id != ''){
		if(confirm("Are you sure?")){
			$.ajax({
				url: "<?=base_url('assets/ajax/form_submit/hrm/leave/performance_approval_rejection_delete_ssubmit.php'); ?>",			
				method: "POST",
				data: {rejected_id:id, approval_note},
				beforeSend:function(){
...
function leave_accept_function(id){
	...
	if(id != ''){
		$.ajax({
			url: "<?=base_url('assets/ajax/form_submit/hrm/leave/performance_approval_rejection_delete_ssubmit.php'); ?>",			
			method: "POST",
			data: {accept_id:id, final_percentage, approval_note},
			beforeSend:function(){
				$('#data-loading').html(data_loading);
			},
			success:function(data){
...
$(document).ready(function() {
    var table_booking = $('#booking_data_table').DataTable({
        ...
        "ajax": "<?=base_url(); ?>assets/ajax/data_table/hrm/leave/employee_performance_approval_hr_datatable.php?month=" + $('#month').val(),
		dom: 'lBfrtip',
