...
<div class="content-wrapper">
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0 text-dark">Sent Requisitions</h1>
				</div> 
				<div class="col-sm-6">
...
function get_damaged_amount(id, type) {
	$.ajax({
		type: "POST",
		url:"<?=base_url('assets/ajax/scm/get_damaged_product_from_department.php');?>",
		data: {id, type},
		beforeSend:function(){
...
function get_stolen_amount(id, type) {
	$.ajax({
		type: "POST",
		url:"<?=base_url('assets/ajax/scm/get_stolen_product_from_department.php');?>",
		data: {id, type},
		beforeSend:function(){
			$('#data-loading').html(data_loading);
		},
...
$('#damaged_product_form').on('submit', function () {
	...
	$.ajax({
		type: "POST",
		url:"<?=base_url('assets/ajax/scm/add_damaged_product_from_department.php');?>",
...
$('#stolen_product_form').on('submit', function () {
	...
	$.ajax({
		type: "POST",
		url:"<?=base_url('assets/ajax/scm/add_stolen_product_from_department.php');?>",
...
$('#receive_product_form').on('submit', function () {
	...
	$.ajax({
		type: "POST",
		url:"<?=base_url('assets/ajax/scm/receive_department_product_from_department.php');?>",
...
function receive_products(requisition_id) {
    $.ajax({
        ...
        url: "<?=base_url()?>assets/ajax/scm/department_product_requisition_from_department.php",
        success: function (data) {
			let info = JSON.parse(data);
			$('#requisition_id').val(requisition_id);
			$('#requisition_button').html(info.button);
...
$(document).ready(function () {
    var purchase_order = $('#requisition_table').DataTable({
		...
		"processing": true,
        "serverSide": true,
        "ajax": "<?=base_url(); ?>assets/ajax/data_table/scm/department_requisition_to_department_datatable.php",
		// rowGroup: {
		// 	dataSrc: 10,
