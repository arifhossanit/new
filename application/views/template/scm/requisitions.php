...
<!-- Issue Slip Modal -->
<div class="modal fade" id="issue_slip">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<div class="modal-header btn-primary">
				<h4 class="modal-title">Issue List</h4>
				<button class="btn btn-info" id="print_button">Print</button>
			</div>
			<div class="modal-body" id="scm_product_issue_list">
...
$('#auto_assign_product').on('submit', function () {
	...
	$.ajax({
        ...
        url: "<?=base_url()?>assets/ajax/scm/auto_assign_product.php",
		beforeSend:function(){					
			$('#data-loading').html(data_loading);
		},
        success: function (data) {
...
let get_issue_slip = (requisition_id) => {
    $.ajax({
        type: 'post',
        data: {requisition_id},
        url: "<?=base_url()?>assets/ajax/scm/scm_issue_slip.php",
        success: function (data) {
			$('#scm_product_issue_list').html(data);
			$('#issue_slip').modal('show');
        }
    });
}
$('#print_button').on("click", function () {	
	$('#scm_product_issue_list').printThis({
		importCSS: true, 
		importStyle: true,
	});
});
$(document).ready(function () {
    var purchase_order = $('#requisition_table').DataTable({
		...
		"processing": true,
        "serverSide": true,
        "ajax": "<?=base_url(); ?>assets/ajax/data_table/scm/requisition_datatable.php",
		// rowGroup: {
		// 	dataSrc: 10,
