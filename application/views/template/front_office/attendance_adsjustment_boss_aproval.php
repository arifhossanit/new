...
<div class="content-wrapper">
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0 text-dark">Attendance Adjustment Aproved By Boss</h1>
				</div> 
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="<?=base_url();?>">Home</a></li>
						<li class="breadcrumb-item"><a href="<?=base_url();?>">Profile</a></li>
						<li class="breadcrumb-item active">Attendance Adjustment Aproved By Boss</li>
					</ol>
				</div> 
			</div> 
		</div> 
			...
			<div class="col-sm-12">
				<div class="card card-primary">
					<div class="card-header">
						<h3 class="card-title">Attendance Adjustment Aproved By Boss</h3>
					</div>
					<div class="card-body">
...
function checked_by_hrm_hr(id, unique_id){
	...
	$.ajax({  
		...
		method:"POST",  
		data:{ boss_accept_checked_id:id, deduction_amount: deduction_amount, note:note},
		beforeSend:function(){					
...
function reject_by_hrm_hr(id, unique_id){
	...
	if(note != ''){
		$.ajax({  
			url:"<?=base_url('assets/ajax/form_model/employee_missing_attendance_info_hrm.php');?>",  
			method:"POST",  
			data:{ boss_reject_checked_id:id, note: note},
			beforeSend:function(){					
				$('#data-loading').html(data_loading);					 
			},
			success:function(data){	
...
$('document').ready(function(){
	...
	var table = $('#candidate_list').DataTable({
		...
		"processing": true,
        "serverSide": true,
		"ajax": "<?=base_url(); ?>assets/ajax/data_table/hrm/pay_roll/employee_missing_attendance_request_boss.php"+condition,
		dom: 'lBfrtip',
        buttons: [			
            {
                extend: 'copy',
