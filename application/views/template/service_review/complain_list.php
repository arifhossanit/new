				...
				<div class="col-sm-10 offset-sm-1">
					<div class="card card-info">
						...
						<div class="card-body">
							...
							INNER JOIN EMPLOYEE ON EMPLOYEE.DEPARTMENT=DEPARTMENT.DEPARTMENT_ID
							INNER JOIN MEMBER_DIRECTORY ON MEMBER_DIRECTORY.ID = COMPLAIN.MEMBER_DIRECTORY_ID
							where complain.status='1' and employee.department='$department'
							...
							?>
									...
									<tbody>
										<?php foreach($pending as $key=>$single){ ?>
											<tr >
												...
												<td class="d-flex">
													<a class="btn btn-xs btn-success ongoing_complain" title="Ongoing" data-id="<?php echo $single->id; ?>"><i class="fas fa-angle-double-right"></i></a>&nbsp;
													<a class="btn btn-xs btn-warning complain_details" data-toggle="modal" data-target="#complain_view" data-id="<?php echo $single->id; ?>"><i class="fa fa-eye"></i></a>
												</td>
											</tr>
										<?php } ?>
									</tbody>
							  ...
							  <div class="tab-pane fade" id="ongoing" role="tabpanel" aria-labelledby="ongoing-tab">
								...
								left join complain_category on complain_category.id=complain.category_id 
								left join department on complain_category.department_id=department.department_id
								left join employee on employee.department=department.department_id
								INNER JOIN MEMBER_DIRECTORY ON MEMBER_DIRECTORY.ID = COMPLAIN.MEMBER_DIRECTORY_ID
								where complain.status='2' and complain.ongoing_from is not null and  employee.department='$department'
								  and member_directory.status='1' group by complain.id
								");
										...
										<tbody>
											<?php foreach($ongoing as $key=>$single){ ?>
												<tr >
													...
													<td class="d-flex">
														<a class="btn btn-xs btn-success fixed_complain" title="Fixed" data-id="<?php echo $single->id; ?>"><i class="fas fa-angle-double-right"></i></a>&nbsp;
														<a class="btn btn-xs btn-success return_pending" title="Return Pending" data-id="<?php echo $single->id; ?>"><i class="fas fa-angle-double-left"></i></a>
													</td>
												</tr>
											<?php } ?>
										</tbody>
							  ...
							  <div class="tab-pane fade" id="fixed" role="tabpanel" aria-labelledby="fixed-tab">
								...
								left join employee on employee.department=department.department_id
								INNER JOIN MEMBER_DIRECTORY ON MEMBER_DIRECTORY.ID = COMPLAIN.MEMBER_DIRECTORY_ID
								where complain.status='0' and  employee.department='$department'
								  and member_directory.status='1' group by complain.id
								");
										...
										<tbody>
											<?php foreach($fixed as $key=>$single){ ?>
												<tr data-id="<?php echo $single->id; ?>">
													...
													<td class="d-flex">
														<a class="btn btn-xs btn-success return_fixed" title="Return Fixed" data-id="<?php echo $single->id; ?>"><i class="fas fa-angle-double-left"></i></a>
													</td>
												</tr>
											<?php } ?>
										</tbody>
