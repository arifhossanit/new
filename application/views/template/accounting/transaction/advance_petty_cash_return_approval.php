<div class="content-wrapper">
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Advance Money Return Aproval & Logs</h1>
          </div> 
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
				...
				<li class="breadcrumb-item active">Advance Money Return Aproval & Logs</li>
            </ol>
          </div> 
        </div> 
      </div> 
				...
				<div class="col-sm-12">
					<div class="card card-success">
						<div class="card-header">
							<h3 class="card-title"><i class="far fa-bed"></i> Advance Money Return Aproval & Logs</h3>
...
<!----End vaiw model-->
<script>
function approval_button(type,id){
	if(type == 1){
		var message = 'Are you Sure want to aproved it?';
	}else{
		var message = 'Are you Sure want to Reject it?';
	}
	if(confirm(message)){
		if(type != '' && id != ''){
			$.ajax({  
				url:"<?=base_url('assets/ajax/form_model/advance_money_return_approval.php'); ?>",  
				method:"POST",  
				data:{ 
					type:type, 
					return_id:id
...
function filter_data_table(){
	...
	var ajax_data4 = "<?=base_url(); ?>assets/ajax/data_table/view_advanced_money_return_aproval_datatable.php"+condition;
...
function purses_item_checkit(transaction_id,amount,name){
	if(confirm('Are you sure want to Checked ('+name+') '+transaction_id+' - BDT '+amount+' ?')){
		if(transaction_id != ''){
			$.ajax({  
				url:"<?=base_url('assets/ajax/form_model/checkit_advance_buied_iteams_options.php'); ?>",  
				method:"POST",  
				data:{ transaction_id:transaction_id },
				beforeSend:function(){					
					$('#data-loading').html(data_loading);
...
function purses_item_checkit_reject(transaction_id,amount,name){
	if(confirm('Are you sure want to Checked ('+name+') '+transaction_id+' - BDT '+amount+' ?')){
		if(transaction_id != ''){
			$.ajax({  
				url:"<?=base_url('assets/ajax/form_model/rejected_advance_buied_iteams_options.php'); ?>",  
				method:"POST",  
				data:{ transaction_id:transaction_id },
				beforeSend:function(){					
					$('#data-loading').html(data_loading);
...
$(document).ready(function() {
    var table_booking = $('#booking_data_table').DataTable({
		...
		"processing": true,
        "serverSide": true,
        "ajax": "<?=base_url(); ?>assets/ajax/data_table/view_advanced_money_return_aproval_datatable.php",
		dom: 'lBfrtip',
        buttons: [			
            {
                extend: 'copy',
