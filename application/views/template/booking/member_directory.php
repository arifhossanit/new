			...
			<div class="row">
				<div class="col-sm-12">					
					<div class="row">
						...
						<div class="col-sm-2">
							<button onclick="show_duplicate_beds()" data-toggle="modal" data-target="#show_duplicate_beds" class="btn btn-info btn-sm">Double Bed Useage</button>
						</div>
					</div>
					<div class="row">
						<div class="col-sm-12">
...
<!----vaiw member profile model-->
<div class="modal fade" id="show_duplicate_beds">
		<div class="modal-dialog modal-xl">
			<div class="modal-content">
				<div class="modal-header btn-warning">
					<h4 class="modal-title">Duplicate Beds</h4>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body" style="max-height:780px;overflow-y:scroll;">	
					<table class="table table-bordered table-hover">
						...
						<tbody id="duplicate_beds_body"></tbody>
					</table>
...
<!----Shifting Receipt model-->
<div class="modal fade" id="shifting_receipt">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<div class="modal-header btn-dark">
				<h4 class="modal-title"> Shifting Member Receipt </h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body" id="shifting_receipt_modal" > </div>
		</div>
	</div>
</div>
<!----End Shifting Receipt model-->

<!-- Card Change Modal -->
<div class="modal" id="confirm_checkout_card_modal">
	<div class="modal-dialog" role="document">
		<form action="" id="checkout_card_confirm_form">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Confirm Checkout Card</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<input type="hidden" id="confirm_card_number">
					<input class="form-control" type="text" id="checkout_card_confirm">
				</div>
...
<!-- Vaccine Card Modal -->
<div class="modal fade" id="vaccine_card_modal">
	<div class="modal-dialog" role="document">
		<form action="" id="vaccine_submit_form">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Upload Vaccine Card</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<input type="hidden" id="vaccine_id_member_id" name="vaccine_id_member_id">
					<input class="form-control-file" type="file" id="vaccine_card" name="vaccine_card">
				</div>
				<div class="modal-footer">
					<button id="vaccine_button" type="submit" class="btn btn-primary">Upload</button>
					<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
				</div>
			</div>
		</form>
...
//Working code to pass last transaction id 

function view_transaction_report(id){
	var transaction_id = id;
	console.log("transaction_id "+transaction_id+" ID");
	if(transaction_id != ''){
		
		$.ajax({  
			url: "<?php echo base_url('assets/ajax/receipt/transaction_details_information.php'); ?>",  
			method: "POST",  
...
$('#vaccine_submit_form').on('submit', () => {
	event.preventDefault();
	var form = $('#vaccine_submit_form')[0];
	var data = new FormData(form);
	$.ajax({
		type: "POST",
		enctype: 'multipart/form-data',
		url:"<?php echo base_url().'assets/ajax/form_submit/submit_vaccine_card.php'; ?>",  
		data: data,
		processData: false,
		contentType: false,
		cache: false,
		timeout: 600000,
		beforeSend:function(){
			$("#vaccine_button").prop("disabled", true);
			$('#data-loading').html(data_loading);
		},
		success:function(data){
			$('#vaccine_card_modal').modal('toggle');
			$('#vaccine_submit_form').trigger("reset");
			$("#vaccine_button").prop("disabled", false);
			$('#data-loading').html('');
...
let show_duplicate_beds = () => {
	$.ajax({  
		url:"<?=base_url('admin/report/duplicate-beds-report');?>",  
		method:"POST",  
		beforeSend:function(){					
			$('#data-loading').html(data_loading);					 
		},
		success:function(data){	
			$('#data-loading').html('');
			console.log(data);
			$('#duplicate_beds_body').html(data);
		}
	});
}

$('#checkout_card_confirm_form').on('submit', (e) => {
	e.preventDefault();
	let confirm_card_number = $('#confirm_card_number').val();
	let card_number = $('#checkout_card_confirm').val();
	if(confirm_card_number === card_number){
		$('#submit_form_button').show();
		$('#confirm_card_button').hide();
		$('#confrim_card_error_message').hide();
	}else{
		$('#confrim_card_error_message').show();
	}
	$('#confirm_checkout_card_modal').modal('hide');
	$('#checkout_card_confirm_form').trigger('reset');
})

$('#confirm_checkout_card_modal').on('hidden.bs.modal', function (e) {
	$('#final_checkout_model').modal('show');
})

let confirm_checkout_card = (card_number) => {
	setTimeout(function() { $('#checkout_card_confirm').focus() }, 100);
	$('#final_checkout_model').modal('hide');
	$('#confirm_checkout_card_modal').modal('show');
	$('#confirm_card_number').val(card_number);
}
...
function get_shifting_receipt(id){
	$.ajax({
		type: "POST",
		data: {change_info_id: id},
		url:"<?=base_url('assets/ajax/package_shifting_details_information.php');?>",
		success:function(data){
			$('#shifting_receipt_modal').html(data);
			$('#shifting_receipt').modal('show');
		}
	});
}
...
let give_vaccine_card = (id) => {
	$('#vaccine_card_modal').modal('toggle');
	$('#vaccine_id_member_id').val(id);	
}
