					...
					<div class="row">
						...
						<div class="col-sm-12" style="margin-top:15px;">
							...
							<div class="row">
								<div class="col-sm-12">
									<div class="form-group">
										<input type="date" class="form-control" min="<?php echo date('Y-m-d'); ?>" max="<?php echo date('Y', strtotime('+1 year')) . '-12-01'; ?>" id="check_new_date" required />
									</div>
								</div>
								<div class="col-sm-12">
									<div class="form-group">
								...
								<div class="col-sm-12">
									<div class="form-group">
										<button id="checkin_date_change_button"  onclick="return submit_change_date()" type="button" class="btn btn-success">Change</button>
									</div>
								</div>
							</div>
						</div>
...
function check_in_date_management(booking_id,checkin_date,member_name,package_category,package_name,branch_name){
	$('#checkin_date_change_button').prop('disabled', false);
	var ck_data;
	ck_data = '<h3>Name: '+member_name+'</h3>';
	ck_data += '<p style="margin-bottom:0px;">Package Category: '+package_category+'</p>';
	ck_data += '<p style="margin-bottom:0px;">Package: '+package_name+'</p>';
	...
	let checkin_datetime = new Date(cvaol);
	let today = new Date();
	checkin_datetime.setHours(0,0,0,0);
	today.setHours(0,0,0,0);
	if(today > checkin_datetime){
		$('#checkin_date_change_button').prop('disabled', true);
	}
...
function submit_change_date(){
	let check_in_date = $('#check_in_date_chm').val();
	let check_in_arr = check_in_date.split('/'); // 0 -> day, 1 -> month, 2 -> year
	let checkin_datetime = new Date(check_in_arr[1] + '-' + check_in_arr[0] + '-' + check_in_arr[2]);
	let today = new Date();
	checkin_datetime.setHours(0,0,0,0);
	today.setHours(0,0,0,0);
	if(today < checkin_datetime){
		alert('Cannot update checkin date for dates in the past!');
		return;
	}
				...
				success:function(data){
					...
					if(value[1] == '1'){
						...
						$.ajax({
							type: "POST",
							enctype: 'multipart/form-data',
							url:"<?= base_url('assets/ajax/form_submit/checkin_api.php') ?>",
							data: {card_number: value[3]},
						});					
					}					
				}
			});
