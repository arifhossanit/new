<?php
	...
	if(!isset($_SESSION['birthday_notification_alert'])){
	$_SESSION['birthday_notification_alert'] = 'true';
	$date = date('d/m', strtotime(date('Y/m/d'). ' + 1 day'));
	$check_employee_birthday = $this->Dashboard_model->mysqlii("select * from employee where date_of_birth like '".$date."/%' and status = '1' and department not in('816905694932688665')");
	$count_employee_birthday = $this->Dashboard_model->mysqlii("select count(*) as total from employee where date_of_birth like '".$date."/%' and status = '1' and department not in('816905694932688665')");
									...
									<div class="<?php if(!empty($col_class)){ echo $col_class; } ?>" id="width_magerment" style="margin-bottom:100px;">
										...
										<center>
											<span>
												...
												<span style="font-size:18px;"><?php echo $row->department_name; ?></span>
											<span>
										</center>
									</div>
									<?php } ?>
