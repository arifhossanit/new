<?php
	...
	if(!isset($_SESSION['aniversary_notification_alert'])){
	$_SESSION['aniversary_notification_alert'] = 'true';
	$date = date('d/m', strtotime(date('Y/m/d'). ' + 1 day'));
	$check_employee_birthday = $this->Dashboard_model->mysqlii("select * from employee where date_of_joining like '".$date."/%' and status = '1' and department not in('816905694932688665')");
	$count_employee_birthday = $this->Dashboard_model->mysqlii("select count(*) as total from employee where date_of_joining like '".$date."/%' and status = '1' and department not in('816905694932688665')");
	if(!empty($check_employee_birthday)){
		function duration_measure($d){
			$a = explode('/',$d);			
			$date1 = $a[2].'-'.$a[1].'-'.$a[0];
			$date2 = date('Y-m-d'); 
		...
		$counter = $count_employee_birthday[0]->total; //
	...
	<div class="modal-dialog modal-xl" style="min-width:100%;margin:0px;">
		<div class="modal-content" style="background:none;">			
			<div class="modal-body close" data-dismiss="modal" id="employee_birthday_popup_body" style="padding:0px;opacity:1.0;">
				<canvas id = 'c' style="width:100%;height:100%;position: absolute; top: 0%; bottom: 0%; left: 0%; right: 0%;opacity: 0.8;"></canvas>
				<div class="row">
					...
					<div class="<?php if(!empty($b_class)){ echo $b_class; } ?>">
						<div class="card" style="margin-top:12%;max-height:755px;overflow-y:scroll;background:rgba(255,255,255,0.5);">
							<div class="card-body">
								<center>
									<h2>Tomorrow's Anniversary</h2>
								</center>
								<div class="row">
									<?php foreach($check_employee_birthday as $row){ ?>
									<div class="<?php if(!empty($col_class)){ echo $col_class; } ?>" id="width_magerment" style="margin-bottom:100px;">
										<img src="<?php echo base_url($row->photo); ?>" style="width:100%;border-radius:100%;" class="set_image_height" alt=""/>
										<center>
											<span>
												<?php echo $row->full_name; ?><br />
												<span style="font-size:15px;"><?php echo $row->department_name; ?></span><br />
												<span style="font-size:15px;"><?php echo duration_measure($row->date_of_joining); ?></span>
											<span>
										</center>
									</div>
									<?php } ?>
