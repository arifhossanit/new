...
<div>
    <div style="margin-top:30px;">
        <center>
            <span style="font-size:20pt;font-weight:900;width:60vw;background-color:#e6437c;color:white;padding:10px;border-radius:30px;">Daily Booking Monthly Report</span>
		</center>
...
<?php 
			...
			if(empty($_POST['branch_id']) && empty($_POST['month'])){
				foreach($d as $i ){			
					...
					$paid = $this->Dashboard_model->mysqlij("SELECT count(id) as nmbr FROM member_directory where data = '".$current_date->format('d/m/Y')."' ");
    ...
    <?php
				foreach($d as $i ){		
					...
					$current_date =  DateTime::createFromFormat('d/m/Y', $i);
					//$current_date->sub(new DateInterval('P1D'));
					$paid = $this->Dashboard_model->mysqlij("SELECT count(id) as nmbr FROM member_directory where data = '".$current_date->format('d/m/Y')."' and branch_id like '%".$_POST['branch_id']."%'  ");
					if(!empty($paid->nmbr) AND $paid->nmbr > 0){ $count_p = $paid->nmbr; }else{ $count_p = '0'; }
					$sum += (int)$count_p;
					$days++;
					$paid_graph_1[] =array( "label" => substr($i,0,6).substr($i,8,2), "y" => $count_p );
				...
				foreach($d as $i ){	
					...
					$current_date =  DateTime::createFromFormat('d/m/Y', $i);
					//$current_date->sub(new DateInterval('P1D'));
					$paid = $this->Dashboard_model->mysqlij("SELECT count(id) as nmbr FROM member_directory where data = '".$current_date->format('d/m/Y')."' and branch_id = '".$_POST['branch_id']."' ");
		...
		<script>
			$('document').ready(function(){
				...
				data: [{
					...
					type: "column", name: "Booking", color: "#e6437c", indexLabel: "{y}", yValueFormatString: "#0.##", showInLegend: true,
